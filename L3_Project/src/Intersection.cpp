...
// L3.1 : Safeguard all accesses to the private members _vehicles and _promises with an appropriate locking mechanism,  ( std::lock_guard<std::mutex> lock(_mutex);)
// that will not cause a deadlock situation where access to the resources is accidentally blocked.

int WaitingVehicles::getSize()
{
    std::lock_guard<std::mutex> lock(_mutex);  // L3.1 
