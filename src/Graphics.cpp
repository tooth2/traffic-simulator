...
void Graphics::drawTrafficObjects()
{
    ...
    for (auto it : _trafficObjects)
    {
        ...
        if (it->getType() == ObjectType::objectIntersection)
        {
            ...
            //cv::rectangle(_images.at(1), cv::Point2d(posx, posy), cv::Point2d(posx+25, posy+25), trafficLightColor, -1);
        }
        else if (it->getType() == ObjectType::objectVehicle)
        {
            ...
            cv::rectangle(_images.at(1), cv::Point2d(posx, posy), cv::Point2d(posx+70, posy+70), vehicleColor, -1);
        }
    }
